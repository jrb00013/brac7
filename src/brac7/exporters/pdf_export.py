...
def _draw_match_box(
    ...
    if is_champion:
        fill = colors.HexColor("#2a3d1a")
        border = colors.HexColor("#ffd700")
    elif not match.is_bye:
        fill = colors.HexColor("#1e2d42")
        border = colors.HexColor("#3d8bfd")
    else:
        fill = colors.HexColor("#2a3a28")
        border = colors.HexColor("#4a7a4a")

    c.setFillColor(fill)
    c.setStrokeColor(border)
    c.setLineWidth(1.2 if not is_champion else 2)
    ...
    if is_champion:
        ...
        c.drawString(x + MATCH_PAD, top - 10, "★ CHAMPION ★")
...
def _draw_bracket_diagram(c: canvas.Canvas, bracket: Bracket) -> None:
    ...
    for r_idx, rnd in enumerate(rounds):
        ...
        for m_idx, match in enumerate(rnd.matches):
            ...
            is_champ = r_idx == len(rounds) - 1 and m_idx == 0
            match_bounds[match.id] = _draw_match_box(
                c, rx, yc, match, bracket, box_w, block_h, is_champ
            )
    ...
    c.drawString(MARGIN, MARGIN + 8, "Each box = one match (top slot vs bottom). Lines show who advances.")
    c.drawString(MARGIN, MARGIN + 1, f"Generated by brac7 — {bracket.title}")
